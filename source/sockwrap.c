...
/* read a whole buffer, for performance, and then return one char at a time */
static ssize_t my_read_select(int fd, char *ptr, long sec)
{
	...
	if (read_cnt <= 0)
	{
		...
		select_ret = select(fd+1, &cset, NULL, NULL, &select_timeout);
		if (select_ret > 0) {
			if ( (read_cnt = read(fd, read_buf, sizeof(read_buf))) < 0)
			{
				if (INTERRUPTED_BY_SIGNAL)
            ...
            printf(("(%s) No response after %lu seconds\n"), prog_name, sec);
            return -1;
        } else{
            if (INTERRUPTED_BY_SIGNAL)
                goto again;
            else {
                err_ret ("(%s) error - select() failed", prog_name);
                return -1;
            }
        }
    }
...
/* NB: Use my_read (buffered recv from stream socket) to get data. Subsequent readn() calls will not behave as expected */
ssize_t my_protocol_readline (int fd, void *vptr, size_t maxlen, long sec) {
	int n;
	ssize_t rc;
	char c = 0, *ptr, prevc;
	...
	{
		prevc = c;
		if ( (rc = my_read_select(fd, &c, sec)) == 1)
		{
			*ptr++ = c;
			if (c == '\n' && prevc == '\r')
				break;	/* newline is stored, like fgets() */
...
ssize_t readline_unbuffered_select (int fd, void *vptr, size_t maxlen, long sec)
{
    int n;
    ssize_t rc;
    char c=0, *ptr, prevc;
...
/* reads exactly "n" bytes from a descriptor */
ssize_t readn_select (int fd, void *vptr, size_t n, long sec) {
    size_t nleft;
    ssize_t nread;
    char *ptr;
    fd_set cset;
...
ssize_t sendn_select (int fd, const void *vptr, size_t n, int flags, long sec)
{
    size_t nleft;
    ssize_t nwritten;
    const char *ptr;
