...
void sig_chld(int signo) {
    ...
    while ((pid = waitpid(-1, &stat, WNOHANG)) > 0) {
        trace(err_msg("(%s) child %d terminated %d by pid:%d\n", prog_name, pid, ++c, getppid()));
    }
    return;
}
...
int main (int argc, char *argv[])
{
    ...
    while (1) {
        /* avoid race condition between signal handler wait and blocking wait */
        trace(err_msg("(%s) waiting for connections ...", prog_name));
        conn_fd = Accept(listen_fd,(SA*) &cliaddr, &cliaddrlen);
        trace(showAddr("Accepted connection from", &cliaddr));
        trace(err_msg("new socket: %d\n", conn_fd));
        if ((childpid = Fork()) == 0) {
            Close(listen_fd);
            while(serve(conn_fd) == 1);
            exit(0);
        } else {
...
void clean_close(int *fd, int reply) {
    ...
    trace(err_msg("(%s) Disconnecting not replying", prog_name));
    ret = close(*fd);
    *fd = -1;
    if (ret != 0) {
